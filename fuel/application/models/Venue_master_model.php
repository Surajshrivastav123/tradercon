...
class Venue_master_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Venue_master_item'; // the name of the record class (if it can't be determined)
	...
	public $serialized_fields = array('images'); // fields that contain serialized data. This will automatically serialize before saving and unserialize data upon retrieving
	...
	{
		parent::__construct('fuel_venue_master'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		// $this->db->select('venue_master.id, venue_master.title, venue_master.published....'); uncomment and  change to the fields to be displayed
	...
	{	
		...
		$fields['images'] = array('display_label' => True,
            'add_extra' => FALSE,
            'repeatable' => TRUE,
              'multiple' => TRUE,
            'required' => TRUE,
            ...
            'title_field' => 'title',
            'fields' => array(
                'sections' => array('type' => 'section', 'display_label' => FALSE, 'label' => 'Gallery'),
                'title' => array('style' => 'width: 500px'),
                'img' => array('label' => 'Image',
                    'class' => 'tab',
                    'folder' => 'images',
...
class Venue_master_item_model extends Base_module_record {
