...
class Advisors_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Advisor'; // the name of the record class (if it can't be determined)
	...
	{
		parent::__construct('fuel_advisors'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		 $this->db->select('advisors.id, advisors.name,advisors.email,advisors.phone_number ,advisors.is_active');  
...
class Advisor_model extends Base_module_record {
