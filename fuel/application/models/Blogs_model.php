...
class Blogs_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Blog'; // the name of the record class (if it can't be determined)
	...
	public $foreign_keys = array('category_id'=>array('categories'=>'BlogCatagory_model'),); // map foreign keys to table models
	...
	{
		parent::__construct('fuel_blogs'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		// $this->db->select('Blogs.id, Blogs.title, Blogs.published....'); uncomment and  change to the fields to be displayed
	...
	{	
		...
		$fields['date_added']= array('type' => 'hidden' );
		$fields['image']= array('required'=>'required',  'label' => 'Image','type' => 'asset','display_preview' => TRUE,'readonly'=>'readonly','hide_options' => TRUE,'encrypt_name' => TRUE,
		'folder'=>'images/blog',
		'is_image' => TRUE,
...
class Blog_model extends Base_module_record {
