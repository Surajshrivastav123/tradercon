...
class Event_sponsors_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Sponsor'; // the name of the record class (if it can't be determined)
	...
	public $foreign_keys = array('event_id'=>array('events'=>'events_master_model'),
                                    'sponsor_id'=>array('sponsors'=>'sponsors_master_model','where' => array()),
                                    'category_id'=>array('event'=>'category_master_model','where' => array())); // map foreign keys to table models
	...
	{
		parent::__construct('fuel_event_sponsors'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		// $this->db->select('sponsors.id, sponsors.title, sponsors.published....'); uncomment and  change to the fields to be displayed
...
class Event_sponsor_model extends Base_module_record {
