...
class Event_speakers_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Event_speaker'; // the name of the record class (if it can't be determined)
	...
	public $foreign_keys = array('event_id'=>array('events'=>'events_master_model'),
                                    'speaker_id'=>array('speakers'=>'speakers_master_model','where' => array()),
	...
	{
		parent::__construct('fuel_event_speakers'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		$col ="event_speakers.".$col;
	  $this->db->select('event_speakers.id,b.title,c.name,event_speakers.published,event_speakers.precedence')
	  ->join('events_master as b', 'b.id=event_speakers.event_id', 'left')
	  ->join('speakers_master as c', 'c.id=event_speakers.speaker_id', 'left');  
	...
	public function _common_query($display_unpublished_if_logged_in = NULL)
	{
		...
		$this->db->order_by('event_speakers.precedence asc');
	}

}

class Event_speaker_model extends Base_module_record {
