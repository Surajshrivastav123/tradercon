...
class Event_pricing_plans_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Event_pricing_plans_item_model'; // the name of the record class (if it can't be determined)
	...
	public $foreign_keys = array('event_id'=>array('events'=>'events_master_model'),
	...
	public $serialized_fields = array('content'); // fields that contain serialized data. This will automatically serialize before saving and unserialize data upon retrieving
	...
	{
		parent::__construct('fuel_event_pricing_plan'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		$this->db->select('event_pricing_plan.id, b.title as event, event_pricing_plan.title,  event_pricing_plan.precedence, price, event_pricing_plan.last_updated, event_pricing_plan.date_added, event_pricing_plan.published')
		->join('events_master as b', 'b.id=fuel_event_pricing_plan.event_id', 'left')
		;  
	...
	{	
		$fields = parent::form_fields($values, $related);
		 $fields['price'] = array('type' => 'currency', 'decimal' => '.', 'currency' => 'Rs.', 'min' => 0, 'required' => true);
		 $fields['content'] = array('display_label' => True,
		 'add_extra' => FALSE,
		 'repeatable' => TRUE,
		   'multiple' => TRUE,
		 'required' => TRUE,
...
class Event_pricing_plans_item_model extends Base_module_record {
