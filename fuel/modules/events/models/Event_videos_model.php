...
class Event_videos_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Event_video'; // the name of the record class (if it can't be determined)
	...
	{
		parent::__construct('fuel_event_videos'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		// $this->db->select('event_videos.id, event_videos.title, event_videos.published....'); uncomment and  change to the fields to be displayed
	...
	/*	$fields['image']= array('required'=>false,  'label' => 'Image','type' => 'asset','display_preview' => TRUE,'hide_options' => TRUE,'encrypt_name' => TRUE,
		//'folder'=>'images/banner_image',
		'is_image' => true,
		'display_overwrite' => FALSE);*/
		$fields['video']= array('required'=>'required',  'label' => 'Video','type' => 'file','display_preview' => TRUE,'hide_options' => TRUE,'encrypt_name' => TRUE,
		'folder'=>'event/video',
		'is_image' => false,
		'display_overwrite' => FALSE);
		return $fields;
...
class Event_video_model extends Base_module_record {
